...
const Waitlist: React.FC = () => {
        ...
        try {
            ...
            if (error) {
                ...
                if (msg.includes("duplicate") || msg.includes("unique")) {
                    // treat as success (already joined)
                    setStep("avatar");
                    setLoading(false);
                    return;
                }

                setErrorMsg("Couldn’t join the waitlist right now. Please try again.");
                setLoading(false);
                return;
            }
    ...
    const handleAvatarSubmit = () => {
        // No DB action here (unless you want to store avatar later using Supabase Storage)
        setLoading(true);
                                ...
                                <AnimatePresence mode="wait">
                                    {step === "email" && (
                                        ...
                                        >
                                            ...
                                            <form onSubmit={handleEmailSubmit} className="space-y-5">
                                                ...
                                                {errorMsg && (
                                                    <div className="text-left text-sm font-semibold text-red-500 px-2">
                                                        {errorMsg}
                                                    </div>
                                                )}
                                    ...
                                    {step === "avatar" && (
                                        ...
                                        >
                                            ...
                                            <p className="text-secondary mb-8 text-center text-lg">
                                                Choose an avatar for your new account.
                                            </p>
                                            ...
                                            <div className="grid grid-cols-4 gap-4 mb-8">
                                                {presetAvatars.map((avatar, i) => (
                                                    <button
                                                        ...
                                                        className={`relative rounded-2xl overflow-hidden aspect-square border-4 ${selectedAvatar === avatar
                                                                ? "border-btn-primary ring-4 ring-btn-primary/30"
                                                                : "border-transparent opacity-60 hover:opacity-100 transition-opacity"
                                                            }`}
                                                    >
                                                        <img src={avatar} alt="" className="w-full h-full object-cover" />
                                                        {selectedAvatar === avatar && (
                                                            <div className="absolute inset-0 bg-btn-primary/40 flex items-center justify-center">
                                            ...
                                            <div className="text-center mb-8">
                                                <div className="text-sm text-muted font-bold uppercase tracking-wider mb-6">
                                                    OR
                                                </div>
                                                <button
                                                    type="button"
                                                    onClick={() => fileInputRef.current?.click()}
                                                    className="flex items-center justify-center gap-3 w-full h-16 border-2 border-dashed border-border-base rounded-2xl text-secondary text-lg font-semibold hover:text-primary hover:border-muted hover:bg-page transition-all"
                                                >
                                            ...
                                            <button
                                                ...
                                                className={`w-full h-16 rounded-2xl font-bold text-lg transition-all flex items-center justify-center gap-3 ${selectedAvatar
                                                        ? "bg-btn-primary text-white hover:bg-btn-hover shadow-xl"
                                                        : "bg-page text-muted cursor-not-allowed border border-border-base"
                                                    }`}
                                            >
                                                {loading ? (
                                                    <div className="w-6 h-6 border-4 border-white/30 border-t-white rounded-full animate-spin" />
                                    ...
                                    {step === "success" && (
                                        ...
                                        >
                                            ...
                                            <h3 className="text-4xl md:text-5xl font-extrabold text-primary mb-4">
                                                You're In!
                                            </h3>
                                            ...
                                            <div className="bg-page rounded-2xl p-6 flex items-center gap-5 text-left border border-border-base">
                                                {selectedAvatar ? (
                                                    <img
                                                        src={selectedAvatar}
                                                        className="w-16 h-16 rounded-full border-2 border-border-base object-cover"
                                                    />
                                                ) : (
